...
// Definition of the ProductDetails functional component
const ProductDetails = () => {
  // Extracting the 'id' parameter from the URL using React Router's 'useParams'
  const { id } = useParams();

  // Finding the product with the given 'id' from the imported product data
  const product = products.find((p) => p.id === parseInt(id));

  // Accessing the 'addToBag' function from the BagContext using the custom hook 'useBag'
  const { addToBag } = useBag();

  // If the product with the given 'id' doesn't exist, display a "Product not found" message
  if (!product) {
  ...
  return (
    <div className="product-details-container container mt-5">
      ...
      <div className="product-details-main">
        {/* Product card with images and information */}
        <div className="product-card" key={product.id}>
          ...
          <div className="product-info">
            ...
            <p className="product-rating" style={{ color: "#12805D" }}>
              {product.rating}
            </p>
            ...
            {/* Button to add the product to the shopping bag */}
            <button className="card-btn" onClick={() => addToBag(product)}>
              {/* SVG icon */}
              <svg
      ...
      <div className="Product-Description">
        ...
        <br />
        <br />
        <p className="Description-Body">{product.specs}</p>
      </div>
      {/* Sidebar containing additional content */}
